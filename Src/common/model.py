...
from Src.config.config import DevelopConfig
...
class UserInfo(BaseModel, Base):
    ...
    diaries = relationship("Diary", backref="user", lazy=True)
    capriccios = relationship("Capriccio", backref="user", lazy=True)
    impressions = relationship("Impression", backref="user", lazy=True)
...
class Diary(BaseModel, Base):
    ...
    __tablename__ = "diary"
    user_id = Column(Integer, ForeignKey("user_info.id"), nullable=False)  # 外键id
    week = Column(String(12))  # 星期
    weather = Column(String(16))  # 天气
    mood = Column(String(32))  # 心情
...
class Capriccio(BaseModel, Base):
    """
    随想
    """

    __tablename__ = "capriccio"
    user_id = Column(Integer, ForeignKey("user_info.id"), nullable=False)  # 外键id
    extract = Column(Text, nullable=False)  # 摘录语句、段落
    content = Column(Text, nullable=False)  # 随想内容
    status = Column(String(12))  # 状态
    zan_times = Column(Integer, default=0)  # 点赞数
    cover = Column(String(80))  # 封面图片
    

class Impression(BaseModel, Base):
    """
    读后感
    """
    ...
    title = Column(String(120), nullable=False)  # 标题
    abstract = Column(Text)  # 摘要
    content = Column(Text, nullable=False)  # 读后感内容
    status = Column(String(12))  # 状态
    zan_times = Column(Integer, default=0)  # 点赞数
    cover = Column(String(80))  # 封面图片
